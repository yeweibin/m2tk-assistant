...
import m2tk.dvb.decoder.descriptor.ContentDescriptorDecoder;
...
public class ContentDescriptorNodeBuilder implements TreeNodeBuilder
{
    private final ContentDescriptorDecoder decoder = new ContentDescriptorDecoder();
    ...
    public MutableTreeNode build(Encoding encoding)
    {
        ...
        {
            int identifier = contentIdentifiers[i];
            node.add(create(String.format("类型描述%d = %s，用户字节 = 0x%02X",
                                          i + 1,
                                          contentDescription(identifier),
                                          decoder.getUserByte(identifier))));
        }

        return node;
    }

    private String contentDescription(int identifier)
    {
        int nibble = (identifier >> 8);
        return contentDescriptions.getOrDefault(nibble, "未定义");
    }
}
